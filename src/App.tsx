...
const DEFAULT_NEW_NOTE_CONTENT = "»õ ¸Þ¸ð\n\nhttps://";
const DEFAULT_PERSONAL_NOTE_CONTENT =
  "°³ÀÎ ¸Þ¸ðÀå\n\n°£´ÜÇÑ ¸Þ¸ð, ºÏ¸¶Å©, ÀÌ¹ÌÁö URLÀ» ¸ð¾ÆµÎ´Â °ø°£ÀÔ´Ï´Ù.\nhttps://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&w=900&q=80";
...
const LEGACY_TEXT_REPLACEMENTS: Array<[string, string]> = [
  ...
  [
    "ê·¸ë£¹ ë©”ëª¨??n\nì£¼ì œë³?ë³´ë“œ?ì„œ ê°ìž ì°¾ì? ë§í¬?€ ?ë£Œë¥??¨ê»˜ ê³µìœ ?´ë³´?¸ìš”.\n?? AI Studio ?ˆí¼?°ìŠ¤ ëª¨ìŒ",
    DEFAULT_GROUP_NOTE_CONTENT
  ],
  ["ê°œì¸ ë©”ë¾î", "°³ÀÎ ¸Þ¸ðÀå"],
  ["ê·¸ë£¹ ë©”ë¾î", "±×·ì ¸Þ¸ðÀå"],
  ["ê°„ë‹¨??ë©”ë¾î, ë¶ë§ˆ?? ?´ë?ì§€ URL??ëª¨ì•„?ëŠ” ê³µê°„?…ë‹ˆ??", "°£´ÜÇÑ ¸Þ¸ð, ºÏ¸¶Å©, ÀÌ¹ÌÁö URLÀ» ¸ð¾ÆµÎ´Â °ø°£ÀÔ´Ï´Ù."],
  ["ì£¼ì œë³?ë³´ë“œ?ì„œ ê°ìž ì°¾ì? ë§í¬?€ ?ë£Œë¥??¨ê»˜ ê³µìœ ?´ë³´?¸ìš”.", "ÁÖÁ¦º° º¸µå¿¡¼­ °¢ÀÚ Ã£Àº ¸µÅ©¿Í ÀÚ·á¸¦ ÇÔ²² °øÀ¯ÇØº¸¼¼¿ä."],
  ["?? AI Studio ?ˆí¼?°ìŠ¤ ëª¨ìŒ", "¿¹: AI Studio ·¹ÆÛ·±½º ¸ðÀ½"],
  ["AI ?´ìŠ¤", "AI ´º½º"]
];
...
const getNoteTitle = (content: unknown) => {
  ...
  const firstLine = cleaned.split("\n").find((line) => line.trim().length > 0) ?? "»õ ¸Þ¸ð";
...
const isDisposableEmptyNote = (note: NoteV2) => {
  ...
  return trimmed === "»õ ¸Þ¸ð" || trimmed === "https://" || trimmed === DEFAULT_NEW_NOTE_CONTENT.trim();
};
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const description =
      selectedBoard?.description?.trim() || "WZD¿¡¼­ ¸Þ¸ð, ¸µÅ©, À§Á¬ÀÌ ´ã±ä º¸µå¸¦ ¸¸µé°í °øÀ¯ÇØº¸¼¼¿ä.";
  ...
  const handleInviteUser = async (profile: BoardUserProfile) => {
    ...
    try {
      ...
      setInviteError("º¸µå ÃÊ´ë¿¡ ½ÇÆÐÇß½À´Ï´Ù. Àá½Ã ÈÄ ´Ù½Ã ½ÃµµÇØ ÁÖ¼¼¿ä.");
    } finally {
      setInviteLoading(false);
    }
  };
      ...
      <div className="pin-app">
        <header className="pin-topbar">
          <div className="topbar-primary">
            <div className="topbar-board-title">
              ...
              <p className="feed-kicker">
                {isSharedView ? "°øÀ¯ º¸µå" : feedMode === "active" ? "°³ÀÎ º¸µå" : "º¸°ü ¸Þ¸ð"}
              </p>
              {canRenameBoard && editingBoardTitle ? (
                ...
                >
                  {feedMode === "active" ? selectedBoard?.title ?? "My Board" : "º¸°ü ¸Þ¸ð"}
                </h1>
              )}
            </div>
            ...
            <div className={`search-shell ${mobileSearchOpen ? "mobile-open" : ""}`}>
              ...
              <input
                ...
                placeholder={feedMode === "active" ? "³» ¸Þ¸ð¿Í ¸µÅ© °Ë»ö" : "º¸°ü ¸Þ¸ð °Ë»ö"}
              />
            </div>
          </div>
          ...
          <div className="topbar-actions">
            ...
            {hasSupabaseConfig ? (
              ...
              <div className="profile-pill muted">·ÎÄÃ ¸ðµå</div>
            )}
          </div>
        </header>
          ...
          <div className="settings-overlay" onClick={() => setSettingsOpen(false)}>
            <section className="settings-panel" onClick={(event) => event.stopPropagation()}>
              <div className="settings-panel-head">
                ...
                <button className="settings-close" onClick={() => setSettingsOpen(false)} aria-label="¼³Á¤ ´Ý±â">
                  ¡¿
                </button>
              </div>
              ...
              <div className="settings-section">
                <div className="settings-section-head">
                  <strong>»èÁ¦µÈ ¸Þ¸ð</strong>
                  <span>{sortedTrashedNotes.length}°³</span>
                </div>
                {sortedTrashedNotes.length === 0 ? (
                  <p className="settings-empty">30ÀÏ ¾È¿¡ º¹±¸ÇÒ ¸Þ¸ð°¡ ¾ø½À´Ï´Ù.</p>
                ) : (
                  <div className="trash-list">
          ...
          <div className="settings-overlay" onClick={() => setInviteOpen(false)}>
            <section className="settings-panel invite-panel" onClick={(event) => event.stopPropagation()}>
              <div className="settings-panel-head">
                ...
                <button className="settings-close" onClick={() => setInviteOpen(false)} aria-label="ÃÊ´ë Ã¢ ´Ý±â">
                  ¡¿
                </button>
              </div>
              ...
              <div className="settings-section invite-search-section">
                <div className="settings-section-head">
                  ...
                  <span>À¯Àú ÀÌ¸ÞÀÏ(¾ÆÀÌµð)·Î °Ë»öÇØ ÃÊ´ëÇÏ¼¼¿ä.</span>
                </div>
                  ...
                  <div className="invite-member-list">
                    {boardMembers.map((member) => (
                      <div className="invite-member-item" key={member.userId}>
                        ...
                        <span className="chip-badge">ÆíÁý °¡´É</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <main className="pin-main" onTouchStart={onBoardTouchStart} onTouchEnd={onBoardTouchEnd}>
                  ...
                  ? cloudSaveState === "saving"
                    ...
                    : cloudSaveState === "saved"
                      ...
                      : cloudSaveState === "error"
                        ? "Å¬¶ó¿ìµå ÀúÀå¿¡ ½ÇÆÐÇß½À´Ï´Ù"
                        : feedMode === "active"
                          ? `${activeNotes.length}°³ÀÇ ÇÉ`
                          : `${archivedNotes.length}°³ÀÇ º¸°ü ¸Þ¸ð`
                  : feedMode === "active"
                    ? `${activeNotes.length}°³ÀÇ ÇÉ`
                    : `${archivedNotes.length}°³ÀÇ º¸°ü ¸Þ¸ð`}
              </span>
          ...
          >
            {loading ? (
              ...
              <div className="feed-empty">
                {feedMode === "active" ? "¾ÆÁ÷ ¸Þ¸ð°¡ ¾ø½À´Ï´Ù." : "º¸°üµÈ ¸Þ¸ð°¡ ¾ø½À´Ï´Ù."}
              </div>
            ) : (
                            ...
                            {!isSharedView && (
                              <div className="pin-actions">
                                ...
                                <button
                                  ...
                                  aria-label={feedMode === "active" ? "¸Þ¸ð ¼öÁ¤" : "¸Þ¸ð º¹±¸"}
                                  title={feedMode === "active" ? "¸Þ¸ð ¼öÁ¤" : "¸Þ¸ð º¹±¸"}
                                >
                                ...
                                <button
                                  ...
                                  aria-label={feedMode === "active" ? "¸Þ¸ð »èÁ¦" : "¿µ±¸ »èÁ¦"}
                                  title={feedMode === "active" ? "¸Þ¸ð »èÁ¦" : "¿µ±¸ »èÁ¦"}
                                >
                                  <span className="pin-icon-glyph">
                                    <TrashIcon />
                                  </span>
                                ...
                                {selected ? (
                                  <>
                                    ...
                                    <textarea
                                      ...
                                      placeholder="¸Þ¸ð, ¸µÅ©, ÀÌ¹ÌÁö URLÀ» ÀÔ·ÂÇÏ¼¼¿ä"
                                      rows={1}
                                    />
                                  </>
                                ) : (
                                  <>
                                    ...
                                    {(!useImageHeroCard || (!hideHoverMetadata && (hasTextPreview || hasLinkPreview))) && (
                                      <p className="pin-body-preview" style={{ fontSize: `${fontSize}px` }}>
                                        {previewText || (noteUrl ? getUrlSnippet(noteUrl) : "¸Þ¸ð¸¦ Å¬¸¯ÇØ¼­ ÆíÁýÇÏ¼¼¿ä.")}
                                      </p>
                                    )}
                                  </>
                                )}
          ...
          <div className="infinite-scroll-status" aria-live="polite">
              ...
              ? "¾Æ·¡·Î ½ºÅ©·ÑÇÏ¸é ¸Þ¸ð°¡ °è¼Ó ·ÎµåµË´Ï´Ù."
              : `${filteredNotes.length}°³ÀÇ ¸Þ¸ð°¡ ¸ðµÎ Ç¥½ÃµÇ¾ú½À´Ï´Ù.`}
          </div>
        </main>
        ...
        {feedMode === "active" && runningDragNoteId && (
          <button
            ...
            onDrop={onTrashDrop}
            aria-label="ÈÞÁöÅëÀ¸·Î »èÁ¦"
            title="ÈÞÁöÅëÀ¸·Î »èÁ¦"
          >
            <span className="floating-trash-icon">??</span>
            <span className="floating-trash-label">ÈÞÁöÅë</span>
          </button>
        )}
      </div>
    </div>
